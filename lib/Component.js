...
export class Component
{
  ...
  get nodeIndex() {
    return Array.prototype.indexOf.call(this.node.parentNode.children, this.node)
  }

  updateChildList(elem, oChildren, nChildren, length) {
    let nIndex, oIndex, oChild, nChild, sibling
    for(nIndex = 0; nIndex < length; nIndex++) {
    // for(nIndex = length - 1; nIndex >= 0; nIndex--) {
      nChild = nChildren[nIndex]
      oChild = elem.__keys.get(nChild.key)
      oIndex = oChild.nodeIndex
      oChild.removeAllListeners()
      nChild.updateElem(oChild)
      console.log(oIndex + ' => ' + nIndex)
      if(oIndex !== nIndex) {
        sibling = oChild.node.parentNode.children[nIndex]
        sibling.before(oChild.node)
        // sibling.after(oChild.node)
      }
    }
  }

  updateChildren(elem) {
    const oChildren = elem?.props.children || []
    ...
    if(elem && elem !== this) {
      this.node = elem.node
      if(this.__keys.size && elem.__keys.size) {
        // return this.updateChildList(elem, oChildren, nChildren, length)
      }
    }
    for(i = 0; i < length; i++) {
      oChild = oChildren[i]
      ...
      if(!oChild) {
        ...
        nChild.props && nChild.componentDidMount()
        continue
      }
  ...
  normalizeChildren(children) {
    ...
    for(const child of [children].flat(Infinity)) {
      ...
      child.key && this.__keys.set(child.key, child)
    }
    return result
  }
  ...
  findAll(component, filter, limit) {
    const result = []
    ...
    for(child of this.props.children) {
      ...
      result.push(...child.findAll(component, filter, limit))
      if(limit && result.length >= limit) {
        break
      }
    }
  ...
  findOne(component, filter) {
    return this.findAll(component, filter, 1)[0] || null
  }
